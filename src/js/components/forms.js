import { removeClasses } from './main-a';
...
function formFieldsInit(options = { viewPass: false }) {
  ...
  if (formFields.length) {
    formFields.forEach(formField => {
      const cleanFormBtn = document.querySelector('[data-clean-form]');

      if (cleanFormBtn) {
        cleanFormBtn.addEventListener('click', function () {
          formValidate.formClean(cleanFormBtn.closest('form'));
        });
      }
    ...
    setTimeout(() => {
      ...
      if (selectOptions.length) {
        selectOptions.forEach(selectOption => {
          selectOption.classList.remove('_active');
